...
def test_scaffold_multi_property_optimization_with_improvement(generator, standard_config):
    ...
    # Create temporary output directory in tests folder to capture initial and final populations
    output_dir = tempfile.mkdtemp(prefix="test_scaffold_", dir="./tests")
    ...
    molecules = generator.optimize(
        ...
        fine_tune_epochs=10,  # Fine-tune model on scaffold-filtered dataset (2 epochs for testing)
        verbose=True,
    ...
    initial_csv = os.path.join(output_dir, 'epoch', 'epoch_0', 'elite_molecules.csv')
    ...
    print(f"   💾 Temporary directory kept for inspection")
