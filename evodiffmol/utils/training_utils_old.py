...
def train_epoch(model, train_loader, optimizer_global, optimizer_local, 
    ...
    with tqdm(total=len(train_loader), desc='Training') as pbar:
        for batch in train_loader:
            ...
            # Apply update_mask to losses if available (scaffold training)
            if update_mask is not None:
                # Ensure update_mask has the correct shape for broadcasting
                ...
                loss_pos_global_mean = loss_pos_global.sum() / num_unmasked
                loss_pos_local_mean = loss_pos_local.sum() / num_unmasked
                loss_node_global_mean = loss_node_global.sum() / num_unmasked
                loss_node_local_mean = loss_node_local.sum() / num_unmasked
                ...
                sum_loss += loss.item()
                sum_loss_pos_global += loss_pos_global_mean.item()
                sum_loss_node_global += loss_node_global_mean.item()
                sum_loss_pos_local += loss_pos_local_mean.item()
                sum_loss_node_local += loss_node_local_mean.item()
            else:
                # Original logic for non-scaffold training
                loss = loss.mean()
                loss.backward()
                orig_grad_norm = clip_grad_norm_(model.parameters(), config.train.max_grad_norm)
                optimizer_global.step()
