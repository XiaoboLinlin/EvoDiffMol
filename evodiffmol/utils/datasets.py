...
class QM403D(InMemoryDataset):
    r"""
        A `Pytorch Geometric <https://pytorch-geometric.readthedocs.io/en/latest/index.html>`_ data interface for :obj:`QM40` dataset
        which is similar to QM9 but with larger molecules (up to 92 atoms) and more diverse atom types.
        It consists of about 163,000 molecules with 12 regression targets:
        ...
        Args:
            root (string): the dataset folder will be located at root/qm40.
            ...
            remove_h (bool, optional): Whether to remove hydrogen atoms from the dataset.
                If True, loads from qm40_without_h/raw/, otherwise from qm40/raw/.
                (default: :obj:`False`)
    """

    def __init__(self, split, condition=None, root='./datasets/QM40', transform=None, pre_transform=None, pre_filter=None, remove_h=False):
        self.split = split
        self.condition = condition
        self.remove_h = remove_h
        
        # Choose data path based on remove_h flag
        if remove_h:
            self.folder = osp.join(root, 'qm40_without_h')
            self.processed_file_suffix = 'qm40_without_h'
        else:
            self.folder = osp.join(root, 'qm40_with_h')
            self.processed_file_suffix = 'qm40_with_h'
            
        self.transform = transform

        super(QM403D, self).__init__(self.folder, transform, pre_transform, pre_filter)

        self.processed_paths[0] = osp.join(self.folder, f'QM40_{self.processed_file_suffix}.pt')
    ...
    def processed_file_names(self):
        if self.condition is None:
            return f'qm40_pyg_{self.split}_{self.processed_file_suffix}.pt'
        else:
            return f'qm40_pyg_{self.split}_{self.condition}_{self.processed_file_suffix}.pt'

    def download(self):
        # QM40 dataset should be manually placed in the correct directory
    ...
    def process(self):
        ...
        for i in tqdm(range(len(N))):
            ...
            # Center the molecular positions so that center of mass is at zero
            R_i = R_i - R_i.mean(dim=0, keepdim=True)
    ...
    def get_half_split(self, data_size, dataset):
        ...
        if dataset == 'qm40_second_half':
            sliced_perm = fixed_perm[data_size // 2:]
        elif dataset == 'qm40_first_half':
...
class QM93D(InMemoryDataset):
    ...
    def __init__(self, split, condition=None, root='./datasets/qm9', transform=None, pre_transform=None, pre_filter=None):
        self.split = split
        self.condition = condition
        ...
        self.processed_paths[0] = './datasets/qm9/QM9.qm9'
        self.data, self.slices = torch.load(self.processed_paths[0], weights_only=False)
...
class General3D(InMemoryDataset):
    r"""
        A general dataset class for any NPZ format dataset (GuacaMol, MOSES, etc.)
        that follows the same structure as QM9/QM40.
        
        Args:
            dataset_name (str): Name of the dataset (e.g., 'guacamol', 'moses')
            split (str): Dataset split ('train', 'valid', 'test')
            root (str): Root directory containing the dataset
            remove_h (bool): Whether to load the 'without_h' version
            transform, pre_transform, pre_filter: Standard PyTorch Geometric options
    """

    def __init__(self, dataset_name, split, root='./datasets', transform=None, pre_transform=None, pre_filter=None, remove_h=False):
        self.dataset_name = dataset_name
        self.split = split
        self.remove_h = remove_h
        
        # Choose data path based on remove_h flag and dataset name
        if remove_h:
            self.folder = osp.join(root, dataset_name, 'without_h')
            self.processed_file_suffix = f'{dataset_name}_without_h'
        else:
            self.folder = osp.join(root, dataset_name, 'with_h')
            self.processed_file_suffix = f'{dataset_name}_with_h'
            
        self.transform = transform

        super(General3D, self).__init__(self.folder, transform, pre_transform, pre_filter)

        self.processed_paths[0] = osp.join(self.folder, f'{dataset_name}_{self.processed_file_suffix}.pt')
        self.data, self.slices = torch.load(self.processed_paths[0], weights_only=False)
    ...
    @property
    def processed_file_names(self):
        return f'{self.dataset_name}_pyg_{self.split}_{self.processed_file_suffix}.pt'

    def download(self):
        # These datasets should be manually processed using process_datasets.py
        pass
    ...
    def process(self):
        ...
        # GuacaMol and MOSES datasets don't have property targets like QM9/QM40
        # They only have molecular structure information
        ...
        for i in tqdm(range(len(N))):
            ...
            # Create data object without property targets
            data = Data(pos=R_i, atom_type=z_i, num_atoms=torch.tensor(N[i]))
...
class General3DOnDemand(Dataset):
    r"""
        A memory-efficient dataset class for large NPZ format datasets (GuacaMol, MOSES, etc.)
        that loads molecules on-demand instead of loading all into memory.
        
        Use this for very large datasets (>500k molecules) where General3D InMemoryDataset causes OOM.
        ...
        Args:
            ...
            root (str): Root directory containing the dataset
            remove_h (bool): Whether to load the 'without_h' version
            transform: Standard PyTorch Geometric transform
    """

    def __init__(self, dataset_name, split, root='./datasets', transform=None, remove_h=False):
        self.dataset_name = dataset_name
        self.split = split
        self.remove_h = remove_h
        self.transform = transform
        ...
        # Load NPZ file directly (no .pt files needed)
        raw_dir = osp.join(self.folder, 'raw')
        npz_path = osp.join(raw_dir, f'{split}.npz')
        
        if not osp.exists(npz_path):
            raise FileNotFoundError(f"Dataset file not found: {npz_path}")
            
        print(f"Loading {dataset_name} {split} dataset (remove_h={remove_h}) with on-demand loading...")
        
        # Load NPZ data (unavoidable but much more memory-efficient than .pt files)
        self.npz_data = np.load(npz_path)
        self.positions = self.npz_data['positions']
        self.charges = self.npz_data['charges'] 
        self.num_atoms = self.npz_data['num_atoms']
        
        print(f"Dataset loaded: {len(self.num_atoms):,} molecules (on-demand loading)")
    ...
    def __getitem__(self, idx):
        """Get a single molecule by index (on-demand loading)."""
        # Get molecule data for this index
        n_atoms = self.num_atoms[idx]
        positions = torch.tensor(self.positions[idx][:n_atoms], dtype=torch.float32)
        atom_types = torch.tensor(self.charges[idx][:n_atoms], dtype=torch.int64)
    ...
    def close(self):
        """Close NPZ file to free memory."""
        if hasattr(self, 'npz_data'):
            self.npz_data.close()
