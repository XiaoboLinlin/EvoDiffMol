...
class AttentionSparse(Attention):
...
class EGNNSparse(MessagePassing):
    ...
    def forward(self, x: Tensor, edge_index: Adj = None,
                ...
                angle_data: List = None, size: Size = None, update_mask: OptTensor = None) -> Tensor:
        """ Inputs:
        ...
        hidden_out, coors_out = self.propagate(edge_index, x=feats, edge_attr=edge_attr_feats,
                                               ...
                                               batch=batch, update_mask=update_mask)
    ...
    def propagate(self, edge_index: Adj, size: Size = None, **kwargs):
        ...
        if self.update_coors:
            ...
            # Apply update mask to coordinate updates (key modification for fixed structures)
            if "update_mask" in kwargs and kwargs["update_mask"] is not None:
                # update_mask: (N,) where 0=fixed, 1=updateable
                coors_delta = coors_delta * kwargs["update_mask"].unsqueeze(-1)
            
            coors_out = kwargs["coors"] + coors_delta
        else:
...
class EGNNSparseNetwork(nn.Module):
    ...
    def forward(self, z, pos, edge_index, edge_attr, batch,
                bsize=None, recalc_edge=None, verbose=0, update_mask=None):
        ...
        for i, layer in enumerate(self.mpnn_layers):
            ...
            if exists(self.global_tokens):
                _, amounts = torch.unique(batch, return_counts)
            ...
            if not is_global_layer:
                x = layer(x, edge_index, edge_attr, batch=batch, size=bsize, update_mask=update_mask)
            else:
                ...
                x = layer[-1](x, edge_index, edge_attr, batch=batch, size=bsize, update_mask=update_mask)
