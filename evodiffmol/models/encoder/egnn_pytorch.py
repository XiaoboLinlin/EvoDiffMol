...
class EGNN(nn.Module):
    ...
    def forward(self, feats, coors, edges=None, mask=None, adj_mat=None, update_mask=None):
        b, n, _ = feats.shape
        device = feats.device
        ...
        if exists(self.coors_mlp):
            ...
            coors_delta = einsum('b i j, b i j c -> b i c', coor_weights, rel_coors)
            
            # Apply update mask to coordinate updates (key modification for fixed structures)
            if exists(update_mask):
                # update_mask: (b, n) where 0=fixed, 1=updateable
                update_mask_expanded = rearrange(update_mask, 'b n -> b n ()')
                coors_delta = coors_delta * update_mask_expanded
            
            coors_out = coors + coors_delta
        else:
            coors_out = coors
...
class EGNNNetwork(nn.Module):
    ...
    def forward(
        ...
        for global_attn, egnn in self.layers:
            ...
            feats, coors = egnn(feats, coors, adj_mat=adj_mat, edges=edges, mask=mask, update_mask=update_mask)
